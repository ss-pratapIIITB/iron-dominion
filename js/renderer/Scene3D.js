...
class Scene3D {
  ...
  _buildTerrain() {
    ...
    for (let row = 0; row < vertsPerSide; row++) {
      for (let col = 0; col < vertsPerSide; col++) {
        ...
        // Height based on terrain type (gives 3D depth with directional light)
        let yHeight = 0;
        if (terrain === TERRAIN.WATER)    yHeight = -0.35;
        else if (terrain === TERRAIN.MOUNTAIN) yHeight = 0.18;
        else if (terrain === TERRAIN.FOREST)   yHeight = 0.05;
        else if (terrain === TERRAIN.SAND)     yHeight = -0.02;
        positions[pi + 1] = yHeight;
  ...
  // ── Trees ──────────────────────────────────────────────────
  _buildTrees() {
    // Remove old tree instances
    this._treeInstances.forEach(m => m.dispose());
    this._treeInstances = [];

    // Create template tree mesh (trunk + canopy merged)
    const trunk = BABYLON.MeshBuilder.CreateCylinder("trunkT", {
      height: 1.2, diameterTop: 0.18, diameterBottom: 0.28, tessellation: 6
    }, this.scene);
    const canopy = BABYLON.MeshBuilder.CreateSphere("canopyT", {
      diameter: 1.4, segments: 5
    }, this.scene);
    canopy.position.y = 1.1;

    // Merge trunk + canopy into single mesh for instancing
    const treeMaster = BABYLON.Mesh.MergeMeshes([trunk, canopy], true, true);
    treeMaster.name = "treeMaster";
    treeMaster.isVisible = false;

    const trunkMat = new BABYLON.StandardMaterial("treeMat", this.scene);
    trunkMat.diffuseColor = new BABYLON.Color3(0.18, 0.38, 0.14);
    trunkMat.specularColor = new BABYLON.Color3(0.02, 0.04, 0.02);
    treeMaster.material = trunkMat;
    treeMaster.receiveShadows = true;
    ...
    for (let ty = 0; ty < MAP_H; ty++) {
      for (let tx = 0; tx < MAP_W; tx++) {
        ...
        const inst = treeMaster.createInstance(`tree_${instCount++}`);
        inst.position.x = tx + 0.5 + (r1 - 0.5) * 0.6;
        inst.position.z = ty + 0.5 + (r2 - 0.5) * 0.6;
        inst.position.y = 0.6; // trunk base above ground
        inst.scaling.setAll(0.7 + r3 * 0.5); // size variety
  ...
  _createBuildingMesh(building) {
    ...
    const stone  = new BABYLON.Color3(0.68, 0.62, 0.52);
    const wood   = new BABYLON.Color3(0.55, 0.38, 0.20);
    const thatch = new BABYLON.Color3(0.72, 0.60, 0.32);
    const roof   = new BABYLON.Color3(0.55, 0.28, 0.15);

    if (type === 'TOWN_CENTER') {
      // Grand keep: multi-tower castle
      addBox("base",     size, 1.4,  size, 0, 0.7, 0, stone);
      addBox("keep",     size*0.7, 1.8, size*0.7, 0, 2.6, 0, stone);
      addBox("roof",     size*0.75, 0.4, size*0.75, 0, 3.6, 0, new BABYLON.Color3(0.3, 0.28, 0.4));
      // 4 corner towers
      const h = size * 0.4;
      [[-h,-h],[h,-h],[-h,h],[h,h]].forEach(([ox, oz], i) => {
        addBox(`tower${i}`, 0.55, 2.8, 0.55, ox, 1.4, oz, stone);
        addBox(`battlements${i}`, 0.6, 0.35, 0.6, ox, 2.9, oz, stone);
      });
      addBox("flag", 0.06, 0.9, 0.06, 0, 4.2, 0, pc);
    } else if (type === 'BARRACKS') {
      addBox("base",  size, 1.0, size, 0, 0.5, 0, stone);
      addBox("walls", size*0.85, 1.2, size*0.85, 0, 1.6, 0, stone);
      addBox("roof",  size*0.9, 0.5, size*0.9,   0, 2.3, 0, wood);
      addBox("flag",  0.06, 0.7, 0.06, 0, 2.8, 0, pc);
    } else if (type === 'ARCHERY_RANGE') {
      addBox("base",  size, 0.8, size, 0, 0.4, 0, wood);
      addBox("walls", size*0.9, 1.0, size*0.9, 0, 1.3, 0, wood);
      addBox("roof",  size*0.95, 0.4, size*0.95, 0, 1.9, 0, thatch);
      addBox("flag",  0.06, 0.6, 0.06, size*0.4, 2.2, size*0.4, pc);
    } else if (type === 'STABLE') {
      addBox("base",  size, 0.8, size, 0, 0.4, 0, wood);
      addBox("roof",  size*0.95, 0.5, size*0.95, 0, 1.7, 0, thatch);
    } else if (type === 'SIEGE_WORKSHOP' || type === 'FACTORY') {
      addBox("base",   size, 1.0, size, 0, 0.5, 0, stone);
      addBox("chimney", 0.4, 1.5, 0.4, size*0.3, 2.0, size*0.3, new BABYLON.Color3(0.3, 0.3, 0.3));
      addBox("chimney2",0.35, 1.2, 0.35, -size*0.25, 1.8, -size*0.25, new BABYLON.Color3(0.3, 0.3, 0.3));
      addBox("roof",    size*0.9, 0.5, size*0.9, 0, 1.2, 0, new BABYLON.Color3(0.4, 0.4, 0.4));
    } else if (type === 'BLACKSMITH' || type === 'WEAPONRY') {
      addBox("base", size, 0.9, size, 0, 0.45, 0, stone);
      addBox("forge",0.6, 1.0, 0.6, size*0.2, 1.4, size*0.2, new BABYLON.Color3(0.25, 0.22, 0.20));
      addBox("roof", size*0.9, 0.4, size*0.9, 0, 1.4, 0, thatch);
    } else if (type === 'MONASTERY') {
      addBox("nave",  size, 1.6, size*0.7, 0, 0.8, 0, stone);
      addBox("tower", size*0.35, 2.5, size*0.35, 0, 2.05, 0, stone);
      addBox("spire", 0.2, 0.8, 0.2, 0, 3.3, 0, new BABYLON.Color3(0.5, 0.45, 0.35));
    } else if (type === 'TOWER') {
      addBox("base",      0.9, 0.6, 0.9, 0, 0.3, 0, stone);
      addBox("shaft",     0.75, 2.0, 0.75, 0, 1.6, 0, stone);
      addBox("top",       0.85, 0.45, 0.85, 0, 2.7, 0, stone);
    } else if (type === 'WALL') {
      addBox("wall", 1.0, 1.4, 0.35, 0, 0.7, 0, stone);
    } else if (type === 'HOUSE') {
      addBox("base", size, 0.8, size, 0, 0.4, 0, wood);
      addBox("roof", size*0.95, 0.6, size*0.95, 0, 1.5, 0, roof);
    } else if (type === 'FARM') {
      addBox("plot", size*0.95, 0.08, size*0.95, 0, 0.04, 0, new BABYLON.Color3(0.45, 0.32, 0.18));
      addBox("fence1", size, 0.25, 0.08, 0, 0.12, size*0.47, new BABYLON.Color3(0.55, 0.4, 0.2));
      addBox("fence2", size, 0.25, 0.08, 0, 0.12, -size*0.47, new BABYLON.Color3(0.55, 0.4, 0.2));
    } else if (type === 'LUMBER_CAMP' || type === 'MINING_CAMP' || type === 'MILL') {
      addBox("base", size, 0.7, size, 0, 0.35, 0, wood);
      addBox("roof", size*0.9, 0.4, size*0.9, 0, 1.1, 0, thatch);
    } else {
      // Generic fallback
      addBox("body", size, 1.0, size, 0, 0.5, 0, stone);
    }

    // Player color stripe on top
    if (type !== 'WALL' && type !== 'FARM') {
      const stripe = addBox("stripe", size * 0.2, 0.12, size * 0.2, 0, 0.02, 0, pc);
